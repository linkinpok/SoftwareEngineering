...
public final class Arrays {
    ...
    /**
     * Searches for the element <i>t</i> in the unsorted array <code>ts</code>.<i> If <code>ts</code>
     * is sorted use java.util.Arrays.binarySearch instead.</i>
     *
     * @param <T>
     ...
     * @return the index of the specified element <tt>t</tt> in the array <tt>ts</tt>. If the item is
     *         not found <tt>-1</tt> is returned.
     */
    private static <T> int indexOf(@NotNull T[] ts, @Null T t) {
     ...
     */
    public static <T> T[] remove(T[] ts, T t, @Null T[] zeroElementArray)
            throws IllegalArgumentException {
     ...
     */
    public static <T> boolean hasElement(T[] ts, @Null T t) {
     ...
     *           <tt>null</tt>.
     * @return the newly created array which contains all elements of <tt>ts</tt> without the element
     *         at the given index. If the newly created array has length zero the given
     *         <tt>zeroElementArray</tt> is returned.
     */
    private static <T> T[] remove(T[] ts, int index, @Null T[] zeroElementArray)
        ...
        T[] newts = (T[]) Array.newInstance(ts.getClass().getComponentType(), ts.length - 1);
    ...
    /**
     * Creates a new array consisting of all elements of both arrays. If one array is empty the other
    ...
    public static <T> T[] add(T[] ts, T[] tn) {
        ...
        @SuppressWarnings("unchecked")
        T[] newts = (T[]) Array.newInstance(ts.getClass().getComponentType(), ts.length + tn.length);
     ...
     *           the source element.
     */
    public static <T> T[] add(T[] ts, @Null T t) {
     ...
     */
    @SuppressWarnings("unchecked") public static <T> T[] add(@NotNull T[] ts, @Null T t, int index) {
     ...
     *
     * @return An int array with length ts.length+1 where the values [0..ts.length) correspond to the
     *         values in ts and where the value with the index ts.length is t.
     */
    public static <T> T[] append(@NotNull T[] ts, T t) {
    ...
    public static int[] add(@NotNull int[] ts, int t, int index) {
     ...
     *         values in ts and where the value with the index ts.length is t.
     */
    public static int[] append(@NotNull int[] ts, int t) {
     ...
     *           the index; must be valid.
     * @return the newly created array which contains all elements of <tt>ts</tt> without the element
     *         at the given index. If the newly created array has length zero the
     *         <tt>emptyIntArray</tt> is returned.
     */
